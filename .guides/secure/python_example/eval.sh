...
# the Python interpreter, so we can use all four cores in a standard
# box and split up the 768 MB of memory (less a small overhead).

# The limit for 1 process is ~650MB, for 2 about 32MB, for 3 about
# 220MB, and for 4 about 160MB.
python3 ../.guides/secure/eval.py -p4,160 ${TIMEOUT} "python3|${PROGNAME}.py" \
